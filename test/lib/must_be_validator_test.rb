...
class MustBeValidatorTest < Minitest::Test
  DateTime.stub :now, DateTime.new(2020, 12, 1, 0, 0, 0) do
    Date.stub :current, Date.new(2020,12,01) do
      Time.stub :now, Time.new(2020, 12, 1, 0, 0, 0) do
        describe "validate_each" do
          ...
          describe "after" do
            ...
            describe "proc execution" do
              let(:clazz) do
                Class.new(User) do
                  validates :birthday, must_be: { after: ->(t) { 1.year.ago } }
                end
              end
              it 'has an error if the date is before the predicate' do
                Time.stub :now, Time.new(2020, 12, 1, 0, 0, 0) do
                  instance = clazz.new(birthday: Date.new(2018,01,01))
                  refute instance.valid?
                  assert_equal ["Birthday : 2018-01-01 is not a valid value. Date cannot be before 2019-12-01."], instance.errors.full_messages
                end
              end
              it 'is okay if not before the predicate' do
                instance = clazz.new(birthday: Time.now)
                assert instance.valid?
                assert instance.errors.empty?
              end
              it 'ignores time component of timestamp unless time is true on the validation' do
                Time.stub :now, Time.new(2021, 12, 1, 0, 0, 0) do
                  instance = clazz.new(birthday: DateTime.new(2018,01,01, 1, 2, 3))
                  refute instance.valid?
                  assert_equal ["Birthday : 2018-01-01T01:02:03+00:00 is not a valid value. Date cannot be before 2020-12-01."], instance.errors.full_messages
                end
              end
            end
